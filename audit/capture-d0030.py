"""
Capture D-0030 Visual system hygiene pass visual proofs.

Mirrors audit/capture-d0029.py CDP pattern but writes to
audit/d0030-{viewport}-after-fullpage.png (NOT overwriting the
D-0029 captures). Captures the WHOLE homepage at desktop
(1280x800) and mobile (393x851) viewports.

Usage:
  audit/capture-d0030.py <port> <url> <out-prefix>
"""
...
async def main() -> None:
    if len(sys.argv) < 4:
        print("usage: capture-d0030.py <port> <url> <out-prefix>")
        sys.exit(2)
    ...
    await shoot("fullpage", beyond=True)
