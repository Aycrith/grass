/**
 * make-divider-v3-webp.ts
 * Convert D-0013 divider v3 candidate PNG → multi-size webp with
 * cream background removed (binary chroma-key) and trimmed to bbox.
 *
 * Source: apps/comfyui/outputs/largo-lawn/grass-v3-img2img/candidate_1_divider_v3_img2img_00001_.png
 * Outputs: apps/web/public/illustrations/divider-v3-{1200,2400}.webp
 *
 * Cream (#FAF6F0) matches --ll-sand-bleached, so the transparent webp
 * sits cleanly on any sand-bleached section background.
 */
import sharp from "sharp";
import * as fs from "node:fs";
import * as path from "node:path";

const SRC = "C:/Users/camer/DEVNEW/GRASS/apps/comfyui/outputs/largo-lawn/grass-v3-img2img/candidate_1_divider_v3_img2img_00001_.png";
const OUT_DIR = "C:/Users/camer/DEVNEW/GRASS/apps/web/public/illustrations";
const OUT_BASENAME = "divider-v3";
...
async function main() {
  ...
  // Build binary alpha: 0 for cream-tolerance pixels, 255 for painted
  // Also detect the bbox of opaque pixels in a separate pass.
  ...
  // Crop to bbox with 4px margin so anti-aliased edges aren't clipped
  const margin = 4;
  ...
  // Now resize to target widths
  for (const targetW of [1200, 2400]) {
